...
class Mains extends CI_Controller {
    ...
    public function admindash(){
        if ($this->session->userdata('admin') == 'in') {
            $this->load->view('dashboard_orders');/*This if statement is to ensure 
            that the admin is logged in so nobody can just type the url to get adin power*/
        }else{
            redirect('/');
        }
    }
    ...
    public function login(){ /*let's send the password and email over to our model!*/
        ...
        if ($login_check) {
            ...
            $this->session->set_userdata('admin','out'); /*setting the admin status 
            to logged out NOTE probably unnecessary*/
            $this->session->set_flashdata('errors','Email/Password combination incorrect');
            redirect('/mains/admin'); /*how do i go just directly to admin without having 
            mains in the url?*/
        }
    }
